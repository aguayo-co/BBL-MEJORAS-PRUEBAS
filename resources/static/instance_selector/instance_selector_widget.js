function create_instance_selector_widget(opts) {
  let widget_root = $("#" + opts.widget_id);
  let field_input = $("#" + opts.input_id);

  let display_edit_link = widget_root.find(
    ".js-instance-selector-widget-display-edit-link"
  );
  let display_markup_wrap = widget_root.find(
    ".js-instance-selector-widget-display-wrap"
  );
  let trigger_button = widget_root.find(".js-instance-selector-widget-trigger");
  let edit_button = widget_root.find(".js-instance-selector-widget-edit");
  let clear_button = widget_root.find(".js-instance-selector-widget-clear");

  if (!widget_root.length)
    throw new Error(
      `Cannot find instance selector widget \`#${opts.widget_id}\``
    );
  if (!display_markup_wrap.length)
    throw new Error(
      `Cannot find instance selector widget display markup wrap in \`#${opts.widget_id}\``
    );
  if (!field_input.length)
    throw new Error(
      `Cannot find instance selector widget field input in \`#${opts.widget_id}\``
    );
  if (!trigger_button.length)
    throw new Error(
      `Cannot find instance selector widget trigger button in \`#${opts.widget_id}\``
    );
  if (!edit_button.length)
    throw new Error(
      `Cannot find instance selector widget edit button in \`#${opts.widget_id}\``
    );
  ...
  window.addEventListener("message", message => {
    if (message.data && message.data.source === opts.embed_id) {
      ...
      if (!object_pk) {
        throw new Error(
          "`object_pk` was not provided by embed. Received: " +
            JSON.stringify(message.data)
        );
      }
      enter_loading_state();
      hide_modal();
      if (Array.isArray(object_pk)) {
        let is_first_widget = true;

        //Validate pks
        object_pk.forEach(function(pk) {
          const lookup_url = `${opts.lookup_url}?${opts.OBJECT_PK_PARAM}=${pk}`;
          $.get({
            url: lookup_url,
            error: function(err) {
              exit_loading_state();
            ...
            success: function(data) {
              exit_loading_state();
              //First widget use normal method
              if (is_first_widget) {
                is_first_widget = false;
              } else {
                $("#id_resources-ADD").click();
                widget_root = $(
                  ".instance_selector_widget .instance-selector-widget--unselected"
                ).last();
                field_input = widget_root.parent().find("input");
                display_edit_link = widget_root.find(
                  ".js-instance-selector-widget-display-edit-link"
                );
                display_markup_wrap = widget_root.find(
                  ".js-instance-selector-widget-display-wrap"
                );
                trigger_button = widget_root.find(
                  ".js-instance-selector-widget-trigger"
                );
                edit_button = widget_root.find(
                  ".js-instance-selector-widget-edit"
                );
                clear_button = widget_root.find(
                  ".js-instance-selector-widget-clear"
                );
              }
              set_value(data);
            }
