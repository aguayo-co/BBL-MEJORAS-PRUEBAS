(function() {
  ...
  if (IS_INSTANCE_SELECTOR_EMBED) {
    const HASH_EMBED_ID = window.location.hash.split(
      "#instance_selector_embed_id:"
    )[1];
    ...
    document.addEventListener("DOMContentLoaded", function() {
      ...
      // Automatic Search
      const search_form = $("#changelist-search");
      const search_input = search_form.find("#id_q");
      let submitTimeoutId;
      ...
      const select_button = $(
        '<li class="instance-selector__select-button">' +
          '<a class="button button-secondary button-small" title="Select this object">Seleccionar</a>' +
          "</li>"
      );
      actions.append(select_button);

      $(document.body).on("click", ".js-instance-selector-target", function(
        event
      ) {
        event.preventDefault();
      ...
      // Select All Button
      const select_all_button = $(".select_all_visible");
      select_all_button.on("click", function(event) {
        event.preventDefault();
        const visible_elements = $(document.body).find("tbody tr");
        let objects_pks = [];
        visible_elements.each(function() {
          let link = $(this).find("a");
          let href = link[0].href;
          if (!href) {
            const actions = visible_elements.find(".actions").has(link);
            href = actions.find("a[href]")[0].href;
          }
          const data = get_data_from_url(href);
          if (!data) {
          ...
          objects_pks.push(data.object_pk);
        });
        handle_object_pk_selected(objects_pks);
      });
    });
